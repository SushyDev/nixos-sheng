# Audio needs all three of: the UCM search path (firmware.nix), WirePlumber
# using UCM rather than its ACP default, and something applying the HiFi verb
# at boot -- without which every PCM open fails with EINVAL.
{
  config,
  lib,
  pkgs,
...
in
{
  ...
  systemd.services.sheng-alsa-ucm = {
    ...
    serviceConfig = {
      ...
      # The verb alone plays silence: the Speaker device's own EnableSequence
      # is what powers up the four CS35L43 amps and loads their DSP firmware.
